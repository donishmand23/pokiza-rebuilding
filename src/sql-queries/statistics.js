...
const SOCIAL_SET_REGISTRATION_STATISTICS = `
	SELECT 
		ss.social_set_id,
		ss.social_set_name,
		ss.social_set_icon,
		(
			...
			LEFT JOIN streets st ON st.street_id = a.street_id
			LEFT JOIN areas ar ON ar.area_id = a.area_id
			WHERE c.social_set_id = ss.social_set_id AND
			CASE
				WHEN ARRAY_LENGTH($1::INT[], 1) > 0 THEN u.branch_id = ANY($1::INT[])
				ELSE TRUE
			END AND
			CASE
				WHEN ARRAY_LENGTH($2::TIMESTAMPTZ[], 1) = 2 THEN (
					ss.social_set_created_at BETWEEN ($2::TIMESTAMPTZ[])[1] AND (($2::TIMESTAMPTZ[])[2] + '1 day'::INTERVAL)
				) ELSE TRUE
			END AND
			CASE
				WHEN ARRAY_LENGTH($3::INT[], 1) > 0 THEN sta.state_id = ANY($3::INT[])
...
export default {
	SOCIAL_SET_REGISTRATION_STATISTICS,
